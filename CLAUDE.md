...
- `window.APP_SYNC_APP = 'ingles'` — the subpage loads the shared `js/lib/sync.js` too, but this prefix goes in front of the sync code on the wire (`/ingles<code>`), so the unchanged worker stores the blob under a separate KV key (the root app keeps the bare `<code>`). The same sync code pasted into both apps therefore keeps their progress apart, with no backend change. The sync prompts/tooltips are Portuguese via `APP_STRINGS` (`sync*` keys, see `STR` in sync.js).
...
- **Sync** (⇅ button, `js/lib/sync.js` + `sync-worker/`): pull-merge-push of `{mastered, strength, daily}` keyed by a secret sync code (the /ingles/ app prefixes its code, see that section); prefs are per-device on purpose. Merging is conservative (mastered unioned, misses kept, streak = min) so syncing can never falsely graduate a card out of Foco. With `SYNC_URL` empty the whole feature is inert.
